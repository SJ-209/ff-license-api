...
app.post('/api/validate-license', async (req, res) => {
    ...
    try {
       ...
       // --- START OF MODIFIED SUCCESS HANDLING ---
        ...
        if (responseData.license_key) console.log("DEBUG: License Object Keys:", Object.keys(responseData.license_key));

        // 3. LOGIC: Identify the Product ID
        if (meta && meta.product_id) {
            ls_product_id = meta.product_id;
        } 
        else if (license_key_data.test_mode === true && ls_status === 'active') {
            console.warn("Bypassing for Test Mode Key.");
            ls_product_id = PRODUCT_ID; 
        }
        else {
            console.error("FAILED: No meta.product_id found. Meta present:", !!meta);
